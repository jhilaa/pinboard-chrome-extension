document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function processGroupsData(groupsData, selectedGroups) {
        ...
        try {
            if (groupsData != undefined) {
                if (groupsData.records.length > 0) {
        ...
        try {
            ...
            chrome.storage.local.set({ testKey: 'testValue' }, function() {
                if (chrome.runtime.lastError) {
                    console.error('Error storing test data:', chrome.runtime.lastError);
                } else {
            ...
            await (() => {
                chrome.storage.local.set({apiData: data}, function () {
                    console.log('Les données ont été stockées.');
                    //******************************
                })
            })
            */
    ...
    function buildPageFromStoredData() {
        // Récupérer les données stockées dans chrome.storage.local
        chrome.storage.local.get(['apiData'], function (result) {
            if (result.apiData) {
                // Utiliser les données pour construire la page
                const data = result.apiData;

                // Exemple de construction de la page avec les données
                const container = document.getElementById('data-container');
                container.innerHTML = ''; // Effacer le contenu précédent

                data.forEach(item => {
                    const div = document.createElement('div');
                    div.textContent = `Item: ${item.name}`; // Adapter en fonction de la structure des données
